...
let selectedWeaponForForge = null; // ✅ 用於鐵匠舖強化武器
...
function goToTown() {
    ...
    locationHtml += `<button onclick="goToBlacksmith()" id="BlacksmithButton">前往鐵匠鋪</button>`;
...
function updateBlacksmithWeaponList() {
    ...
    const eligibleWeapons = player.inventory.filter(w => w.rarity && w.name !== "木棍");

    if (eligibleWeapons.length === 0) {
        listDiv.innerHTML = "<p>你沒有可強化的武器。</p>";
        return;
    }
    ...
    eligibleWeapons.forEach((weapon, index) => {
        listDiv.innerHTML += `
            <button onclick="selectWeaponForForge(${index})">
                【${weapon.name}】（${weapon.rarity}，攻擊 +${weapon.power}）
            </button><br>
        `;
    });
}
...
function selectWeaponForForge(index) {
    const weapon = player.inventory.filter(w => w.rarity && w.name !== "木棍")[index];
    if (!weapon) return;
    ...
    document.getElementById("selectedWeaponInfo").innerHTML = `
        ...
        <p>目前攻擊力：+${weapon.power}</p>
        <p>強化費用：${price} 金幣</p>
        <button onclick="upgradeWeapon()">強化武器</button>
    `;
}
...
function upgradeWeapon() {
    ...
    if (player.gold >= price) {
        ...
        log(`你成功強化了【${selectedWeaponForForge.name}】，攻擊力提升至 +${selectedWeaponForForge.power}！`);
        updateBlacksmithWeaponList();
        document.getElementById("selectedWeaponInfo").innerHTML = "";
        updateStatus();
        savePlayerData();
...
function goToWarriorVillage() {
    ...
    let locationHtml = `
        <strong>你目前在：</strong> 勇士之村<br>
         ...
         <button onclick="goToBlacksmith('warrior')" id="BlacksmithButton">前往鐵匠鋪</button>
...
function goToPerion() { // 將 Perion 用作墮落城市的名稱
    ...
    let locationHtml = `
        ...
        <button onclick="goToBlacksmith('perion')" id="BlacksmithButton">前往鐵匠鋪</button>
...
function winBattle(monsterName) {
    ...
    if (droppedGold > 0) {
        ...
        log(`💰 獲得了 ${droppedGold} 金幣！`);
        alert(`恭喜！你獲得了 ${droppedGold} 金幣！`); // 金幣掉落視窗
    } else {
        log("沒有獲得金幣。");
    }
